...
var Internal = {
  ...
  appVersion: "",
  osVersion: "",
  isAppVersionGreatThan: function(c) {
    if ((typeof c == "string") && (c.length > 0)) {
      var b = parseFloat(c);
      var a = parseFloat(Internal.appVersion);
      if (a - b >= 0) {
        return true
      }
    }
  ...
  appVersionNotSupportCallback: function(a) {
    var b = {
      tagname: "app_version_too_low",
      start_version: a,
      app_version: Internal.appVersion
    };
    CtripTool.app_log(JSON.stringify(b));
    window.app.callback(b)
  },
  paramErrorCallback: function(a) {
    var b = {
      tagname: "app_param_error",
      description: a
    };
    CtripTool.app_log(JSON.stringify(b));
    window.app.callback(b)
  ...
  makeParamString: function(a, c, d, b) {
    if (!Internal.isNotEmptyString(a) || !Internal.isNotEmptyString(c)) {
      return ""
    }
    if (!d) {
      d = {}
    ...
    d.service = a;
    d.action = c;
    d.callback_tagname = b;
    return JSON.stringify(d)
  },
  makeURLWithParam: function(a) {
    if (a == null) {
      a = ""
    }
    a = encodeURIComponent(a);
    return __CTRIP_URL_PLUGIN + a
  },
  callWin8App: function(a) {
    window.external.notify(a)
  }
};

function __bridge_callback(a) {
  a = decodeURIComponent(a);
  var b = JSON.parse(a);
  if (b != null) {
    if (b.param != null && b.param.hasOwnProperty("platform")) {
      ...
      Internal.appVersion = b.param.version;
      Internal.osVersion = b.param.osVersion
    }
    return window.app.callback(b)
  }
  return -1
}

function __writeLocalStorage(b, a) {
  if (Internal.isNotEmptyString(b)) {
...
var CtripTool = {
  app_is_in_ctrip_app: function() {
    ...
    if (b.indexOf("CtripWireless") > 0) {
      Internal.isAndroid = true;
      a = true
    } else {
      if ((b.indexOf("iPhone") > 0 || b.indexOf("iPad") > 0 || b.indexOf("iPhone")) && b.indexOf("Safari") < 0) {
        a = true;
        Internal.isIOS = true
      }
    }
  ...
  app_log: function(b, a) {
    CtripUtil.app_log(b, a)
  }
};
var CtripUtil = {
  app_log_event: function(a) {
    if (Internal.isNotEmptyString(a)) {
      var b = {};
      b.event = a;
      paramString = Internal.makeParamString("Util", "logEvent", b, "log_event");
      if (Internal.isIOS) {
  ...
  app_call_phone: function(a) {
    if (!a) {
      a = ""
    }
    var c = {};
    c.phone = a;
    paramString = Internal.makeParamString("Util", "callPhone", c, "call_phone");
    if (Internal.isIOS) {
      var b = false;
      if (Internal.isNotEmptyString(a)) {
  ...
  app_back_to_last_page: function(b, a) {
    var c = {};
    if (!b) {
      b = ""
    }
    c.callbackString = b;
    c.isDeleteH5Page = a;
    paramString = Internal.makeParamString("Util", "backToLast", c, "back_to_last_page");
    if (Internal.isIOS) {
      url = Internal.makeURLWithParam(paramString);
      Internal.loadURL(url)
    } else {
  ...
  app_locate: function(a) {
    var b = {};
    b.is_async = a;
    paramString = Internal.makeParamString("Locate", "locate", b, "locate");
    if (Internal.isIOS) {
      url = Internal.makeURLWithParam(paramString);
      Internal.loadURL(url)
    } else {
  ...
  app_refresh_nav_bar: function(a) {
    if (Internal.isNotEmptyString(a)) {
  ...
  app_open_url: function(d, a, h) {
    ...
    g.title = h;
    g.targetMode = a;
    paramString = Internal.makeParamString("Util", "openUrl", g, "open_url");
    if (Internal.appVersion) {
      if (Internal.isIOS) {
        url = Internal.makeURLWithParam(paramString);
        Internal.loadURL(url)
      } else {
      ...
      var b = (c.indexOf("Android") > 0) && (c.indexOf("CtripWireless") > 0);
      if (b) {
        try {
          window.Util_a.openUrl(paramString)
        } catch (f) {
  ...
  app_cross_package_href: function(a, c) {
    var b = {};
    if (!a) {
      a = ""
    }
    ...
    b.path = a;
    b.param = c;
    paramString = Internal.makeParamString("Util", "crossPackageJumpUrl", b, "cross_package_href");
    if (Internal.isIOS) {
      url = Internal.makeURLWithParam(paramString);
      Internal.loadURL(url)
    } else {
  ...
  app_check_app_install_status: function(b, a) {
    var c = {};
    if (!b) {
      b = ""
    }
    ...
    c.openUrl = b;
    c.packageName = a;
    paramString = Internal.makeParamString("Util", "checkAppInstallStatus", c, "check_app_install_status");
    if (Internal.isIOS) {
      url = Internal.makeURLWithParam(paramString);
      Internal.loadURL(url)
    } else {
  ...
  app_refresh_native_page: function(b, a) {
    var c = {};
    if (!b) {
      b = ""
    }
    ...
    c.pageName = b;
    c.jsonStr = a;
    paramString = Internal.makeParamString("Util", "refreshNativePage", c, "refresh_native_page");
    if (Internal.isIOS) {
      url = Internal.makeURLWithParam(paramString);
      Internal.loadURL(url)
    } else {
  ...
  app_copy_string_to_clipboard: function(c) {
    var a = "5.3";
    if (!Internal.isAppVersionGreatThan(a)) {
      Internal.appVersionNotSupportCallback(a);
      return
    }
    var b = {};
    if (!c) {
    ...
    b.copyString = c;
    paramString = Internal.makeParamString("Util", "copyToClipboard", b, "copy_string_to_clipboard");
    if (Internal.isIOS) {
      url = Internal.makeURLWithParam(paramString);
      Internal.loadURL(url)
    } else {
  ...
  app_call_system_share: function(d, c) {
    var a = "5.3";
    if (!Internal.isAppVersionGreatThan(a)) {
      Internal.appVersionNotSupportCallback(a);
      return
    ...
    b.imageRelativePath = d;
    b.text = c;
    paramString = Internal.makeParamString("Util", "callSystemShare", b, "call_system_share");
    if (Internal.isIOS) {
      url = Internal.makeURLWithParam(paramString);
      Internal.loadURL(url)
    } else {
  ...
  app_download_data: function(a, d) {
    var b = "5.3";
    if (!Internal.isAppVersionGreatThan(b)) {
      Internal.appVersionNotSupportCallback(b);
      return
    }
    var f = {};
    if (!a) {
    ...
    var e = Internal.makeParamString("Util", "downloadData", f, "download_data");
    if (Internal.isIOS) {
      var c = Internal.makeURLWithParam(e);
      Internal.loadURL(c)
    } else {
      if (Internal.isAndroid) {
        window.Util_a.downloadData(e)
      } else {
        if (Internal.isWinOS) {
          Internal.callWin8App(e)
        }
      }
    }
  },
  app_open_other_app: function(d, c, b) {
    var a = "5.3";
    if (!Internal.isAppVersionGreatThan(a)) {
      Internal.appVersionNotSupportCallback(a);
      return
    ...
    f.jsonParam = c;
    f.url = b;
    var e = Internal.makeParamString("Util", "openOtherApp", f, "open_other_app");
    if (Internal.isIOS) {
      var b = Internal.makeURLWithParam(e);
      Internal.loadURL(b)
    } else {
      if (Internal.isAndroid) {
        window.Util_a.openOtherApp(e)
      } else {
        if (Internal.isWinOS) {
          Internal.callWin8App(e)
        }
  ...
  app_set_toolbar_hidden: function(b) {
    var a = "5.4";
    if (!Internal.isAppVersionGreatThan(a)) {
      Internal.appVersionNotSupportCallback(a);
      return
    ...
    var c = {};
    c.isHidden = b;
    paramString = Internal.makeParamString("Util", "setToolBarHidden", c, "set_toolbar_hidden");
    if (Internal.isIOS) {
      url = Internal.makeURLWithParam(paramString);
      Internal.loadURL(url)
    } else {
  ...
  app_log: function(b, a) {
    ...
    c.log = b;
    c.result = a;
    paramString = Internal.makeParamString("Util", "h5Log", c, "log");
    if (Internal.isIOS) {
      url = Internal.makeURLWithParam(paramString);
      Internal.loadURL(url)
    } else {
  ...
  app_send_pipe_request: function(c, e, b) {
    var a = "5.4";
    if (!Internal.isAppVersionGreatThan(a)) {
      Internal.appVersionNotSupportCallback(a);
      return
    ...
    d.header = e;
    d.data = b;
    paramString = Internal.makeParamString("Util", "sendPipeRequest", d, "send_pipe_request");
    if (Internal.isIOS) {
      url = Internal.makeURLWithParam(paramString);
      Internal.loadURL(url)
    } else {
...
var CtripPay = {
  app_pay_by_alipay: function(a) {
    ...
    var d = {};
    d.orderId = a;
    var c = Internal.makeParamString("Pay", "payByAliPay", d, "pay_by_alipay");
    if (Internal.isIOS) {
      url = Internal.makeURLWithParam(c);
      Internal.loadURL(url)
    } else {
      if (Internal.isAndroid) {
        window.Pay_a.payByAliPay(c)
      } else {
        if (Internal.isWinOS) {
          Internal.callWin8App(c)
        }
      }
    }
  }
